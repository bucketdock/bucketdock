...
export default function ConnectionFormModal({ open, onClose, initial }: Props) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      toast.error(`Failed to save: ${friendlyError(err)}`);
    } finally {
      setSaving(false);
    }
  }
        ...
        <div>
          ...
          >
            ...
            <InfoHint tone="info" label="Where is this stored?">
              ...
              (readable only by your macOS user). The secret key is{" "}
              <strong>not</strong> stored there.
            </InfoHint>
          </Label>
          <Input
            id="conn-access-key"
        ...
        <div>
          <Label htmlFor="conn-secret" className="flex items-center gap-1.5">
            ...
            <InfoHint tone="security" label="How is my secret stored?">
              Stored in the macOS <strong>Keychain</strong> (encrypted, gated by
              your login password). Never written to disk in plain text and
              never sent anywhere except directly to your S3 endpoint to sign
              API requests via SigV4.
            </InfoHint>
          </Label>
          <Input
            id="conn-secret"
          ...
          {endpoint.trim().toLowerCase().startsWith("http://") && (
            <div className="mt-2 flex gap-2 rounded-md border border-amber-300 dark:border-amber-800 bg-amber-50 dark:bg-amber-950/40 px-2.5 py-2 text-[11px] leading-snug text-amber-800 dark:text-amber-300">
              ...
              <span>
                Insecure endpoint. <code className="font-mono">http://</code>{" "}
                sends credentials in plain text — only use on a trusted local
                network.
              </span>
            </div>
          )}
        ...
        <div>
          <Label htmlFor="conn-filter" className="flex items-center gap-1.5">
            ...
            <InfoHint tone="info" label="When to use this">
              For <strong>scoped tokens</strong> (e.g. a Cloudflare R2 token
              limited to specific buckets), enter the bucket name(s) explicitly
              here — comma or space separated. The app will skip the
              account-wide <code>ListBuckets</code> call (which scoped tokens
              cannot perform) and use these names directly. Leave blank to
              auto-list all buckets.
            </InfoHint>
          </Label>
          <Input
            id="conn-filter"
