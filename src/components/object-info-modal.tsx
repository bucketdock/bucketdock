...
const META_KEY_RE = /^[a-z][a-z0-9-]{0,63}$/; // eslint-disable-line @typescript-eslint/no-unused-vars
...
interface Props {
  ...
  objectKey: string;
  onSaved?: () => void;
  /** "info" hides editing UI; "edit" auto-focuses the editable headers. */
  mode?: "info" | "edit";
}
...
export function ObjectInfoModal({
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={mode === "edit" ? `Edit headers · ${fileName}` : fileName}
        ...
        <div className="flex flex-col gap-5 overflow-y-auto">
          ...
          <section>
            ...
            <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1.5 text-sm">
              ...
              <dd className="truncate font-mono text-xs break-all">
                {objectKey}
              </dd>
              ...
              <dd>
                {metadata.last_modified
                  ? format(
                      new Date(metadata.last_modified),
                      "yyyy-MM-dd HH:mm:ss",
                    )
                  : "—"}
              </dd>
              ...
              {mode === "info" && metadata.content_type && (
                <>
                  <dt className="text-neutral-500">Content-Type</dt>
                  <dd className="truncate">{metadata.content_type}</dd>
                </>
              )}
            </dl>
          </section>
